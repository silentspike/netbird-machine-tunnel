...
func (am *DefaultAccountManager) AddPeer(ctx context.Context, accountID, setupKey, userID string, peer *nbpeer.Peer, temporary bool) (*nbpeer.Peer, *types.NetworkMap, []*posture.Checks, error) {
	...
	if setupKey == "" && userID == "" && !addedByMTLS && !peer.ProxyMeta.Embedded {
	...
	// === MACHINE-TUNNEL-FORK START ===
	var peerAddConfig *peerAddAuthConfig
	if addedByMTLS {
		// mTLS-authenticated machine peer - bypass normal auth flow
		...
		peerAddConfig = &peerAddAuthConfig{
			AccountID: accountID,
			Ephemeral: false,
		}
	} else {
