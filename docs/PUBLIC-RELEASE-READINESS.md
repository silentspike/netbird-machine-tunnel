...
| Post-merge main CI | PASS / final gate | Main push CI for the feature launch commit `0acb92e1` completed 12/12 push workflows successfully. The Linux workflow was recovered by rerunning only the cancelled failed jobs after two unit jobs hung. Go Condition #2 covers the current protected `main` commit at the moment of the visibility flip. |
